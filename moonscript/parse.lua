...
local build_grammar = wrap_env(debug_grammar, function(root)
  ...
  local g = P({
    ...
    Statement = pos(Import + While + With + For + ForEach + V("Switch") + Return + Local + Export + BreakLoop + Ct(ExpList) * (Update + Assign) ^ -1 / format_assign) * Space * ((key("if") * Exp * (key("else") * Exp) ^ -1 * Space / mark("if") + key("unless") * Exp / mark("unless") + CompInner / mark("comprehension")) * Space) ^ -1 / wrap_decorator,
    ...
    Assign = sym("=") * (Ct(With + If + V("Switch")) + Ct(TableBlock + ExpListLow)) / mark("assign"),
    ...
    SimpleValue = If + Unless + V("Switch") + With + ClassDecl + ForEach + For + While + Cmt(Do, check_do) + sym("-") * -SomeSpace * Exp / mark("minus") + sym("#") * Exp / mark("length") + sym("~") * Exp / mark("bitnot") + key("not") * Exp / mark("not") + TblComprehension + TableLit + Comprehension + FunLit + Num,
